...
public class AppointmentRecord {
    ...
    public AppointmentRecord(String patientName, String patientAddress, String patientContact,
                             String service, LocalDate date, String time,
    ...
    // Check conflict, ignoring the appointment currently being edited (pass null for new bookings)
    public static boolean hasConflict(AppointmentRecord editing, LocalDate date, String time, String staff) {
        for (AppointmentRecord appointment : appointments) {
            if (appointment == editing) continue;
            if (appointment.status.equals("Confirmed")
                    && appointment.date.equals(date)
                    && appointment.time.equals(time)
                    && appointment.staff.equals(staff)) {
                return true;
            }
        }
        return false;
    ...
    public static void cancelAppointment(Scanner input) {
        ...
        while (true) {
            System.out.print("Cancel appointment for " + selected.patientName
                    + " on " + selected.date.format(DATE_FMT) + "? (Y/N): ");
